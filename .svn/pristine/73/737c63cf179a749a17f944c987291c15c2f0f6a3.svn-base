<template>
    <div class="goodsdetail">
        <div class="banner">
          <threed-max v-if="!!goodsDetail.threeDModelObjUrl && params=='model'" :objUrl="goodsDetail.threeDModelObjUrl" :mtlUrl ="goodsDetail.threeDModelMtlUrl" :tdProp="goodsDetail.threeDModelProp" :MetalMat="goodsDetail.threeDModelMetalMat"></threed-max>
          <!-- <div class="threed" v-if="params=='model'">
              <iframe src="http://192.168.1.66:8033/yaoshikou/" frameborder="0"></iframe>
          </div> -->
          <mt-swipe :auto="4000" v-if="params=='pic'">
            <mt-swipe-item v-for="(item,index) in detailImg" :key="index"><img :src="item" alt=""></mt-swipe-item>
          </mt-swipe>
          <div class="change-btn" v-if="goodsDetail.threeDModelObjUrl != null">
        ...
        <p class="goods-price">¥{{goodsDetail.goodSalePrice | two}} <span>销量：{{goodsDetail.goodSalesCount}}</span></p>
        ...
        <mt-popup v-model="popupVisible" closeOnClickModal="false" popup-transition="popup-fade" position="bottom" @touchmove.prevent >
            ...
            <div v-if="modal == 'choose'" class="choose-wrp">
                ...
                <div class="buy" v-if="modelDetail.skuQuantity==0">
                    <div class="bkgray jcar fl" >加入购物车</div>
                    <div class="bkgray nbuy fl " >当日告罄</div>
                </div>
    ...
    created(){
         this.$http.get('/api/good/goodinfo?id=' + this.$route.query.id).then(res => {
             if(res.data.message=="成功"){
                    ...
                    this.goodImg = res.data.data.threeDModelMetalMat;
             }else{
                 alert('没有此商品');
...
.bkgray{
    background:linear-gradient(to right, #b3b3b3,#999);
}
</style>
