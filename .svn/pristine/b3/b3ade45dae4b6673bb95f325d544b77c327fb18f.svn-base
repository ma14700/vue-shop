      ...
      <div class="order-good">
            ...
            <ul>
                <li v-for="n in orderInfo.orderItem" :key="n.id">
                    ...
                    <div class="gd-wrp">
                        ...
                        <p class="order-gdinfo">{{proData(n.itemSku)}}</p>
                    </div>
                </li>
            </ul>
        </div>
            ...
            <ul class="reason-detail">
               <!-- <li @click="handelClick('status')">货物状态<i class="iconfont icon-right "></i><span v-text="statusText"></span></li> -->
                <li @click="handelClick('type')">退货类型<i class="iconfont icon-right "></i><span v-text="typeText"></span></li>
                <li @click="handelClick('reason')">退货原因<i class="iconfont icon-right "></i><span v-text="reasonText"></span></li>
            </ul>

            <ul class="reason-detail">
                <li v-if="ischoosetype==1">退款金额<p>¥{{orderInfo.orderTotalAmount | two}}</p></li>
                <li>退货说明<input type="text" placeholder="必填" v-model="hushStatus"></li>
            </ul>
            ...
            <div class="btm-submit" @click="goodsubmit()">
             ...
             <mt-popup v-model="popupVisible" popup-transition="popup-fade" position="bottom">
                 <div v-if="modal == 'reason'" class="coupon-wrp">
                    <h2>退货原因</h2>
                    <ul>
                        <li v-for="(item,index) in refundreason" :key="index">{{item.reason}}<i class="iconfont " @click="choosereason(item,index)" :class="item.ischoosed?'icon-duigou':'icon-quan'"></i></li>

                    </ul>
                    <p @click="confirmO()">确认</p>
                 </div>
                 <div v-if="modal == 'type'" class="coupon-wrp">
                    <h2>退货类型</h2>
                    <ul>
                        <li v-for="(item,index) in refundtype" :key="index">{{item.type}}<i class="iconfont " @click="choosetype(item,index)" :class="item.ischoosed?'icon-duigou':'icon-quan'"></i></li>
...
export default {
  ...
  methods: {
    ...
    goodsubmit(){
      ...
      //     });
          ...
          this.$http.post('/api/neworder/rejected/'+this.$route.query.id,{
            ...
            ReturnGoodsType:this.ischoosetype,
          }).then(res=>{
    ...
    choosereason(item,index) {
      this.reasonText = this.refundreason[index].reason;
      this.refundreason.forEach(function(i) {
    ...
    choosetype(item,index){
      this.ischoosetype = index;
      this.typeText = this.refundtype[index].type;
      this.refundtype.forEach(function(i) {
                this.$set(i, 'ischoosed', false);
            }, this);
      this.$set(item, 'ischoosed', true);
    },
