<template>
  <div class="orderdetail">
      <div class="problem" v-if="orderdetail.orderStatus==  -100">
          <i class="iconfont icon-tishi"></i>
          <p>系统管理员对您的订单进行了关闭处理，如有疑问，请联系客服。</p>
      </div>

      <ul class="order-info" v-if=" orderdetail.orderStatus>100">
            <li class="returnGoods">
                <h2><span>{{orderdetail.orderStatusRemark}}</span><i class="iconfont icon-right" @click="goReDetail(orderdetail.id)"></i></h2>
            </li>
      </ul>
        <div class="order-address1" v-if="transList.length>0" @click="watchtrans()">
            <div >{{transList[0].context}}</div>
            <div >{{transList[0].time}}</div>
            <div class="allow"><i class="iconfont icon-right"></i></div>
            <!-- <i class="iconfont icon-yishouhuo"></i> -->
        </div>
        <div class="order-address">
            <h2><i class="iconfont icon-shouhuoren"></i> 收&ensp;货&ensp;人：{{receiveAddress.receiverName}}</h2>
            <h2><i class="iconfont icon-shouji-copy"></i>手&ensp;机&ensp;号：{{receiveAddress.mobile}}</h2>
            <h2><i class="iconfont icon-dingwei1"></i>收货地址：{{receiveAddress.address}}</h2>
            <p></p>
        </div>
        <div class="user-msg">
            <i class="iconfont icon-xiaoxi"></i> 买家留言：<p v-if="orderdetail.buyerComments">{{orderdetail.buyerComments}}</p>
            <p v-else>留言为空</p>
        </div>
        <div class="invoice-msg" v-if="orderdetail.invoiceType!=0">
            <p>发票信息</p>
            <p>发票类型：<span v-text="orderdetail.invoiceType==2?'企业':'个人'"></span></p>
            <p >发票抬头：{{orderdetail.invoiceTitle}}</p>
            <p v-if="orderdetail.invoiceType==2">企业税号：{{orderdetail.dutyNumber}}</p>
        </div>
        <div class="order-good">
            <h2>
                <img :src="orderdetail.supplierLogo" alt="">
                {{orderdetail.supplierName | dot}}
                <span>{{orderdetail.orderStatusRemark}}</span>
            </h2>
            <ul>
                <li v-for="(item,index) in orderdetail.orderItem" :key="index" @click="goDetail(item.itemId)">
                    <img :src="item.itemPicUrl" alt="">
                    <div class="gd-wrp">
                         <p class="order-gdname"><em>{{item.itemName}}</em><span>¥{{item.itemUnitPrice | two}}</span></p>
                        <p class="order-gdinfo">
                            <em>{{proData(item.itemSku)}}</em>
                            <span>×{{item.itemCount}}</span></p>
                    </div>
                </li>
            </ul>
            <p v-if="canApplySvc(orderdetail.orderStatus,orderdetail.payTime)">
                <span @click="goRefund(orderdetail.id)" >退款/退货</span>
            </p>
        </div>
        <div class="manager-no">理财经理号<span v-text="!!orderdetail.financialManagerNo?orderdetail.financialManagerNo:'未填写'"></span></div>
        <ul class="order-price order-info">
            <li>
                商品总价<span>¥{{orderdetail.orderTotalAmount | two}}</span>
            </li>
            <li>
                活动优惠<span v-text="orderdetail.couponAmount?'－¥'+orderdetail.couponAmount.toFixed(2):'－¥ 0.00'"></span>
            </li>
            <li>
                订单总计<span>¥{{(orderdetail.orderTotalAmount-orderdetail.couponAmount) | two }}</span>
            </li>
        </ul>
        <ul class="order-info" >
            <li >
                订单编号：{{orderdetail.orderNo}}
            </li>
            <li v-if="orderdetail.orderStatus>1">
                银行交易号：{{orderdetail.acqSsn}}
            </li>
            <li v-for="i in orderdetail.orderStatusTimeline" :key="i.id">
                <span v-if="i.afterChangeStatus == 1">创建时间：{{i.happendTime | date}}</span>
                <span v-else-if="i.afterChangeStatus == 2">付款时间：{{i.happendTime | date}}</span>
                <span v-else-if="i.afterChangeStatus==100 || i.afterChangeStatus==105">交易成功：{{i.happendTime | date}}</span>
               <span v-else-if="i.afterChangeStatus==-1 || i.afterChangeStatus==-2 || i.afterChangeStatus==-100">交易关闭：{{i.happendTime | date}}</span>
            </li>

        </ul>
        <p class="tip-text" v-if="orderdetail.orderStatus == 3 && orderdetail.orderDelivery">订单自动确认收货时间：{{delaytime
 | date}}</p>
        <form id="payForm" method="post" :action="proxyConfig" @submit.prevent="submitToPay">
            <input type="hidden" :value="paymentSign.plain" name="Plain">
            <input type="hidden" :value="paymentSign.signature" name="Signature">
            <input type="hidden" :value="paymentSign.transName" name="transName">
        </form>
        <div class="footSubmit" v-if="orderdetail.orderStatus == 4" @click="pushSubmit()">去评价</div>
        <div style="height:1.6rem"></div>
        <div class="order-status" v-if="orderdetail.orderStatus == 1">
            <span @click="cancleOrder()">取消订单</span>
            <span class="red" @click="payOrder(orderdetail.orderNo)" v-if="isOrderPay">立即支付</span>
            <span class="red" v-if="!isOrderPay">支付中</span>
        </div>
        <div class="order-status" v-else-if="orderdetail.orderStatus == 2 || orderdetail.orderStatus == 3 || orderdetail.orderStatus == 202 || orderdetail.orderStatus == 103">
            <i v-if="orderdetail.orderStatus == 2 && DateMinus(orderdetail.payTime) == 0">当天24点前可自行取消订单</i>
            <span @click="cancleOrder()" v-if="orderdetail.orderStatus == 2 && DateMinus(orderdetail.payTime) == 0">取消订单</span>
            <span @click="delayTime" v-if="isDelay"> 延长收货</span>
            <span @click="watchtrans()" v-if="orderdetail.orderStatus == 3 ||orderdetail.orderDelivery != null">查看物流</span>
            <span class="red" v-if="orderdetail.orderStatus ==3 ||orderdetail.orderDelivery != null" @click="confirmReceipt()">确认收货</span>
        </div>
        <div class="order-status" v-if="orderdetail.orderStatus == 203">
            <span class="red" @click="setReturnGoodsExpress()" >录入退货信息</span>
        </div>
        <mt-popup v-model="returnGoodsExpress" popup-transition="popup-fade" position="bottom" @touchmove.prevent>
                <div class="returnGoodsExpress">
                  <h2>录入退货信息</h2>
                  <p><span>快递公司：</span><input type="text" id="expressComName" v-model="comName"></p>
                  <p><span>快递单号：</span><input type="text" id="expressPostNo" v-model="postNo"></p>
                  <p><i class="iconfont icon-tishi"></i>请核对您的快递单号以及快递公司</p>
                  <div @click="confirmInfo()">确定</div>
                </div>
        </mt-popup>
  </div>
</template>
<script>
import { Toast } from "mint-ui";
import { MessageBox } from "mint-ui";
import { Popup } from "mint-ui";
import { setStore, getStore, removeStore } from "../../config/mUtils";
export default {
  data() {
    return {
      orderdetail: [],
      receiveAddress: [],
      transList: [],
      returnGoodsExpress: false,
      comName: null,
      postNo: null,
      paymentSign: {
        plain: "",
        signature: "",
        transName: ""
      },
      proxyConfig: null,
      delaytime: "",
      isDelay: null,
      isOrderPay:true,
    };
  },
  beforeRouteLeave(to, from, next) {
    var select = getStore("selectOrderType") || 0;
    if (to.path == "/order") {
      location.href = "/order?selected=" + select;
      return;
    }
    next();
  },
  computed: {},
  created() {
    this.proxyConfig = require("../../../config/payProxy").payUrl;
    this.getOrder();
  },
  methods: {
    delayTime() {
      var _this = this;
      MessageBox({
        title: "提示",
        message: "确认延长收货吗",
        showCancelButton: true,
        confirmButtonText: "确定",
        closeOnClickModal: false
      }).then(action => {
        if (action == "confirm") {
          _this.$http
            .post("/api/neworder/delay/" + _this.$route.query.id)
            .then(res => {
              if (res.data.success) {
                _this.isDelay = false;
                _this.delaytime = res.data.data;
              }
              alert(res.data.message);

              //   window.location.reload();
            });
        }
      });
    },
    DateMinus: function(sDate) {
      var sdate = new Date(
        sDate
          .replace(/T/g, " ")
          .substring(0, 10)
          .replace(/-/g, "/")
      );
      var now = new Date();
      var days = now.getTime() - sdate.getTime();
      var day = parseInt(days / (1000 * 60 * 60 * 24));
      return day;
    },
    canApplySvc: function(state, paytime) {
      console.info(1);
      if (state == 2) {
        return this.DateMinus(paytime) > 0;
      }
      var canState = [2, 3, 202, 103];
      console.info(canState.indexOf(state));
      return canState.indexOf(state) > -1;
    },
    goDetail(id) {
      this.$router.push({ path: "/goodsdetail", query: { id } });
    },
    pushSubmit() {
      this.$router.push({
        path: "/submit",
        query: { id: this.$route.query.id }
      });
    },
    confirmInfo() {
      if (this.comName == null || this.comName == "") {
        Toast({
          message: "请输入快递公司",
          position: "bottom",
          duration: 1.5e3
        });
      } else if (this.postNo == null || this.postNo == "") {
        Toast({
          message: "请输入快递单号",
          position: "bottom",
          duration: 1.5e3
        });
      } else {
        this.$http
          .post("/api/neworder/rejected/express/" + this.$route.query.id, {
            OrderId: this.$route.query.id,
            RejectedExpressName: this.comName,
            RejectedPostNo: this.postNo
          })
          .then(res => {
            console.log(res.data);
            let _this = this;
            if (res.data.success) {
              MessageBox({
                title: "录入信息",
                message: res.data.message,
                showCancelButton: false,
                confirmButtonText: "完成",
                closeOnClickModal: false
              }).then(action => {
                _this.$router.push({
                  path: "/order",
                  query: { selected: "0" }
                });
              });
            } else {
              MessageBox({
                title: "录入信息失败",
                message: "请核对快递公司以及快递单号",
                showCancelButton: false,
                confirmButtonText: "完成",
                closeOnClickModal: false
              }).then(action => {});
            }
          });
      }
    },
    confirmReceipt() {
      var _this = this;
      MessageBox({
        title: "确认收货",
        message:
          "<p style='font-size:14px;line-height:0.4rem;text-align:center;margin-top:10px;'>请在收到货后再点击确认收货，否则您可能钱货两空。如果您想退款/退货，请返回后在订单详情中申请退款/退货。</p>",
        showCancelButton: true,
        confirmButtonText: "确定",
        closeOnClickModal: false
      }).then(action => {
        if (action == "confirm") {
          _this.$http
            .post("/api/neworder/confirmReceipt/" + _this.$route.query.id)
            .then(res => {
              if (res.data.success) {
                Toast({
                  message: "已确认收货",
                  position: "bottom",
                  duration: 1.5e3
                });
                window.location.reload();
              }
            });
        }
      });
    },
    setReturnGoodsExpress() {
      this.returnGoodsExpress = true;
    },
    //跳转运输详情
    watchtrans() {
      this.$router.push({
        path: "/tranparcel",
        query: { id: this.$route.query.id, postNo:this.orderdetail.orderDelivery.postNo ,postName:this.orderdetail.orderDelivery.expressName,imgUrl:this.orderdetail.orderItem[0].itemPicUrl,transList:this.transList}
      });
    },
    getOrder() {
      this.$http
        .post("/api/neworder/detail/" + this.$route.query.id)
        .then(res => {
          this.orderdetail = res.data.data;
          if (this.orderdetail.orderDelivery) {
            this.delaytime = this.orderdetail.orderDelivery.autoReceiptTime;
            this.isDelay = !this.orderdetail.orderDelivery
              .hasDelayedReceiptTime;
          }

          this.receiveAddress = res.data.data.receiveAddress;
          if (this.orderdetail.orderDelivery) {
            this.$http
              .post("/api/express", {
                ComCode: "auto",
                PostNo: this.orderdetail.orderDelivery.postNo
              })
              .then(res => {
                console.log(res);
                this.transList = res.data.data;
              });
          }
        });
    },
    cancleOrder() {
      var _this = this;
      var msg = null;
      var id = this.orderdetail.id;
      console.info(this.orderdetail.orderStatus);
      if (this.orderdetail.orderStatus == 2) {
        msg =
          "确认取消订单么?<p style='font-size:14px;line-height:0.4rem;text-align:center;margin-top:10px;'>所退货款将在48小时内到账</p>";
      } else {
        msg = "确认取消订单么?";
      }
      MessageBox.confirm(msg, "").then(action => {
        this.$http.post("/api/neworder/cancel/" + id).then(res => {
          console.log(res.data.success);
          if (res.data.success) {
            MessageBox({
              title: "取消订单",
              message: res.data.message,
              showCancelButton: true,
              confirmButtonText: "确定",
              closeOnClickModal: false
            }).then(action => {
              if (action == "confirm") {
                _this.$router.push({
                  path: "/order",
                  query: { selected: "0" }
                });
              }
            });
          } else {
            MessageBox({
              title: "取消订单",
              message: res.data.message,
              showCancelButton: true,
              confirmButtonText: "确定",
              closeOnClickModal: false
            }).then(action => {});
          }
        });
      });
    },
    payOrder() {
      this.isOrderPay = false;
      var _this = this;
      this.$http
        .post("/api/neworder/pay", {
          OrderNo: this.orderdetail.orderNo
        })
        .then(res => {
          this.isOrderPay = true;
          if (res.data.success) {
            this.paymentSign = res.data.data;
            MessageBox({
              title: "确认支付",
              message: "确认支付：¥ " + this.orderdetail.orderTotalAmount.toFixed(2),
              showCancelButton: true,
              confirmButtonText: "确定",
              closeOnClickModal: false
            }).then(action => {
              if (action == "confirm") {
                _this.submitToPay();
              }
            });
          } else {
            MessageBox({
              title: "结算异常",
              message: res.data.message,
              showCancelButton: true,
              confirmButtonText: "确定",
              closeOnClickModal: false
            }).then(action => {});
          }
        }).catch(err=>{
          this.isOrderPay = false;
          if(err){
            this.isOrderPay = true;
          }
        });
    },
    submitToPay() {
      document.getElementById("payForm").submit();
    },
    proData(pro) {
      pro = pro.split(";");
      let data = [];
      for (let i = 0; i < pro.length; i++) {
        data.push(pro[i].split(":")[1]);
      }
      return data.join(";");
    },
    goRefund(id) {
      this.$router.push({ path: "/refund", query: { id } });
    },
    goReDetail(id) {
      let path = null;
      if (this.orderdetail.orderReturnGoods != null) {
        path = "/refundgooddetail";
      } else {
        path = "/refunddetail";
      }
      this.$router.push({ path: path, query: { id } });
    }
  },

  filters: {
    two: function(value) {
      if (!value) {
        return "";
      }
      return value.toFixed(2);
    },
    date(value) {
      if (!value) {
        return "";
      }
      let va = value.replace(/T/g, " ").substring(0, 19);
      return va;
    },
    dot: function(value) {
      if (!value) {
        return value;
      } else {
        if (value.length > 8) {
          return value.slice(0, 8) + "...";
        } else {
          return value;
        }
      }
    }
  }
};
</script>
<style>
/* messagebox */
.mint-msgbox-confirm {
  color: #ec1019;
  font-size: 0.4rem;
}
.mint-msgbox-cancel {
  font-size: 0.4rem;
}
.mint-toast {
  z-index: 2010 !important;
}
</style>

<style scoped>
.problem {
  overflow: hidden;
  background: #ec1019;
  padding: 0.32rem;
  box-sizing: border-box;
  color: #fff;
}
.problem > i {
  display: block;
  font-size: 1rem;
  float: left;
  margin-right: 0.32rem;
}
.problem > p {
  font-size: 0.4rem;
  line-height: 0.54rem;
}
.manager-no {
  padding: 0.32rem;
  background: #fff;
  margin-bottom: 0.2rem;
  font-size: 0.36rem;
}
.manager-no > span {
  float: right;
}
.footSubmit {
  width: 10rem;
  height: 1.333333rem;
  background: #ec1019;
  margin: 0 auto;
  line-height: 1.333333rem;
  font-size: 0.48rem;
  text-align: center;
  color: #fff;
  position: fixed;
  bottom: 0%;
}
.mint-popup {
  width: 10rem;
}
.allow {
  width: 0.533333rem;
  height: 0.533333rem;
  position: absolute;
  right: 3%;
  top: 50%;
  margin-top: -0.27rem;
}
.order-status {
  position: fixed;
  width: 10rem;
  bottom: 0;
  background: #fff;
  text-align: right;
  padding: 0.32rem;
  box-sizing: border-box;
  overflow: hidden;
}
.order-status > i {
  float: left;
  line-height: 0.68rem;
  color: #acacac;
}
.order-status > span {
  display: inline-block;
  height: 0.68rem;
  border: 1px solid #acacac;
  border-radius: 0.28rem;
  padding: 0 0.4rem;
  color: #acacac;
  line-height: 0.68rem;
  margin-left: 0.24rem;
  box-sizing: border-box;
  min-width: 2.24rem;
  text-align: center;
}
.order-status > span.red {
  border-color: #ec1019;
  color: #ec1019;
}
.orderdetail {
  background: #f7f7f7;
}
.invoice-msg {
  overflow: hidden;
  line-height: 0.58rem;
  font-size: 0.32rem;
  background: #fff;
  margin-bottom: 0.2rem;
  color: #333;
  padding: 0.32rem;
  box-sizing: border-box;
}
.user-msg {
  overflow: hidden;
  line-height: 0.54rem;
  font-size: 0.32rem;
  padding: 0.32rem;
  background: #fff;
  margin-bottom: 0.2rem;
  box-sizing: border-box;
}
.user-msg > i {
  color: #333;
  position: relative;
  top: 2px;
}
.user-msg > p {
  color: #a9a9a9;
  width: 7.1rem;
  line-height: 0.54rem;
  float: right;
  word-wrap: break-word;
}
.order-address1 {
  box-sizing: border-box;
  padding: 0.2rem 0.3rem;
  background: #fff;
  border-bottom: 1px solid #f4f4f4;
  font-size: 0.38rem;
  position: relative;
  line-height: 0.6rem;
}
.order-address1 > i {
  position: absolute;
  top: 0.8rem;
}
.order-address1 > div:first-child {
  color: #60cc83;
  width: 85%;
}
.order-address1 > div:nth-of-type(2) {
}
.order-address {
  background: #fff;
  margin-bottom: 0.2rem;
  overflow: hidden;
  padding-top: 0.32rem;
  position: relative;
}
.order-address > h2 {
  font-size: 0.38rem;
  padding: 0 0.32rem;
  height: 0.6rem;
  line-height: 0.6rem;
}
.order-address > h2:nth-of-type(1) > i {
  margin-right: 0px;
}
.order-address > h2 > i {
  margin-right: 3px;
}
.order-address > span {
  color: #999;
  font-size: 0.36rem;
  padding: 0 0.32rem;
  line-height: 0.54rem;
}
.order-address > p {
  margin-top: 10px;
  background: url(../../assets/home/caixian.png);
  background-size: 100% 100%;
  width: 100%;
  height: 6px;
}
.order-address > i {
  position: absolute;
  right: 0.32rem;
  top: 0.54rem;
  font-weight: bold;
  color: #999;
}
.order-good {
  overflow: hidden;
  padding: 0 0.32rem;
  background: #fff;
  margin-bottom: 0.2rem;
}
.order-good > h2 {
  font-size: 0.36rem;
  padding: 0.24rem 0;
  line-height: 0.68rem;
  border-bottom: 1px solid #dedede;
}
.order-good > h2 > img {
  width: 0.64rem;
  height: 0.64rem;
  border-radius: 50%;
  box-sizing: border-box;
  border: 1px solid #dedede;
  float: left;
  margin-right: 0.32rem;
}
.order-good > h2 > span {
  float: right;
  color: #ec1019;
}

.order-good > ul > li {
  padding: 0.24rem 0;
  overflow: hidden;
}
.order-good > ul > li > img {
  width: 1.728rem;
  height: 1.44rem;
  float: left;
}
.order-good > ul > li .gd-wrp {
  float: left;
  font-size: 0.36rem;
  width: 7.432rem;
  margin-left: 0.2rem;
  height: 1.44rem;
  line-height: 0.65rem;
}
.gd-wrp > p > em {
  max-width: 5.5rem;
  display: inline-block;
  overflow: hidden;
  white-space: nowrap;
  text-overflow: ellipsis;
}
.order-good > p {
  border-top: 1px solid #dedede;
  padding: 0.16rem 0;
  box-sizing: border-box;
  color: #adadad;
  overflow: hidden;
}

.order-good > p > span {
  display: block;
  padding: 0 0.4rem;
  height: 0.56rem;
  float: right;
  text-align: center;
  border: 1px solid #adadad;
  line-height: 0.56rem;
  border-radius: 0.28rem;
}
.gd-wrp .order-gdname {
  color: #333;
}
.gd-wrp .order-gdname > span {
  float: right;
  color: #ec1019;
}
.gd-wrp .order-gdinfo {
  color: #999;
  line-height: 0.6rem;
}
.gd-wrp .order-gdinfo > span {
  float: right;
  color: #333;
}
.tip-text {
  width: 100%;
  padding: 0.32rem;
  box-sizing: border-box;
  color: #afafaf;
  font-size: 0.36rem;
  background: #fff;
}
.order-info {
  width: 100%;
  padding: 0.32rem;
  overflow: hidden;
  background: #fff;
  box-sizing: border-box;
  margin-bottom: 0.2rem;
}
.order-info > li {
  color: #afafaf;
  font-size: 0.36rem;
  line-height: 0.54rem;
}

.order-info > li.returnGoods h2 {
  color: #1d63bd;
  /* padding-bottom: 0.2rem; */
}

.order-info > li.returnGoods > h2 > i {
  float: right;
  color: #333;
}
.order-info > li.returnGoods > p {
  overflow: hidden;
  margin-top: 0.2rem;
}
.order-info > li.returnGoods > p > em {
  display: inline-block;
  /* float: right; */
  word-wrap: break-word;
  width: 7.55rem;
}
.order-price > li > span {
  float: right;
}
.order-price > li:last-child {
  color: #333;
  font-size: 0.4rem;
  margin-top: 0.2rem;
}
.order-price > li:last-child > span {
  color: #ec1019;
  font-size: 0.48rem;
}
.returnGoodsExpress {
  width: 100%;
  box-sizing: border-box;
  padding: 0.4rem 0.32rem 1.5rem;
  position: relative;
  font-size: 0.4rem;
}
.returnGoodsExpress h2 {
  margin-top: 0.2rem;
  margin-bottom: 0.4rem;
  text-align: center;
}
.returnGoodsExpress p {
  line-height: 1.4rem;
  font-size: 0.36rem;
  margin: 0.2rem 0;
}
.returnGoodsExpress > p:nth-of-type(3) {
  font-size: 0.32rem;
  line-height: 0.54rem;
  color: #999;
}
.returnGoodsExpress > p:nth-of-type(3) > i {
  margin-right: 2px;
  position: relative;
  top: 2px;
}
.returnGoodsExpress > p > input {
  background: #eee;
  border: none;
  border-radius: 3px;
  height: 1.2rem;
  width: 7.15rem;
  padding: 0 0.2rem;
}
.returnGoodsExpress div {
  line-height: 1.2rem;
  background-color: #ec1019;
  position: absolute;
  left: 0;
  bottom: 0;
  width: 100%;
  color: #fff;
  text-align: center;
  font-size: 0.4rem;
}
</style>


