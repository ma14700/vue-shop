        ...
        <mt-tab-container-item id="n" v-if="orderList.length>0" v-for="n in [0,1,2,3,4]" :key="n"  v-show="selected == n" >
            ...
            <li v-for="(item,index) in orderList[selected]" :key="index" >
                    ...
                    <div class="bottom-tip" v-if="item.orderStatus==1">
                        ...
                        <span @click="payOrder(item)" v-if="!!!item.isPay">立即支付</span>
                        <span v-if="!!item.isPay">支付中</span>
                    ...
                    <div class="bottom-tip" v-else-if="item.orderStatus == 2 || item.orderStatus == 3 || item.orderStatus == 202 || item.orderStatus == 103">
                        <span @click="goDetail(item)">查看详情</span>
                        <span @click="confirmReceipt(item.id)" v-if="item.orderStatus == 3 ||item.orderStatus == 202">确认收货</span>
                        <span @click="watchtrans(item.id)" v-if="item.orderStatus == 3 ||item.orderStatus == 202">查看物流</span>
                        <span @click="cancleOrder(item)" v-if="item.orderStatus == 2 && DateMinus(item.payTime) == 0">取消订单</span>
                    </div>
                    <div class="bottom-tip" v-else >
                        <span @click="goDetail(item)">查看详情</span>
                        <em  style="line-height: .56rem;" v-if="item.orderStatus == 2 && DateMinus(item.payTime) == 0">24点前可取消</em>
                        <span @click="cancleOrder(item)" v-if="item.orderStatus == 2 && DateMinus(item.payTime) == 0">取消订单</span>                    </div>
                    </div>
  ...
    },
    ...
    methods:{
        ...
        confirmReceipt(id){
            ...
            MessageBox({
                title: "确认收货",
                message: "<p style='font-size:14px;line-height:0.4rem;text-align:center;margin-top:10px;'>请在收到货后再点击确认收货，否则您可能钱货两空。如果您想退款/退货，请返回后在订单详情中申请退款/退货。</p>",
                showCancelButton: true,
                confirmButtonText: "确定",
                closeOnClickModal: false
            }).then(action => {
                if (action == "confirm") {
                    _this.$http.post('/api/neworder/confirmReceipt/' + id).then(res=>{
                        if(res.data.success){
                            Toast({
                                message: '已确认收货',
                                position: 'bottom',
        ...
        cancleOrder(item){
            ...
            if(item.orderStatus==2){
                msg ="确认取消订单?<p style='font-size:14px;line-height:0.4rem;text-align:center;margin-top:10px;'>所退货款将在48小时内到账</p>"
            }else{
                msg = "确认取消订单么?"
            }
