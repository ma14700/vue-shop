<template>
    <div class="submitorder">
        ...
        <div class="order-good" v-for="(group,index) in orderGoodList1" :key="index">
            <h2>
                <img :src="group.goodSupplierLogo" alt="">
                {{group.goodSupplierName}}
            </h2>
            <ul v-if="group.shopCartItems">
        ...
        <ul class="other-wrp"  v-if="couponPostResult ||  couponPostResult1">
            <li @click="handelClick('coupon')">活动优惠<i class="iconfont icon-right"></i><i v-text="brandcoupon"></i></li>
        </ul>
        <ul class="other-wrp">
        ...
        <ul class="other-wrp">
            <li>理财经理号<input type="text" placeholder="选填" v-model="BuyManager"><span>{{checktext}}</span></li>
        ...
        <p class="couponamout" v-if="couponPostResult ||couponPostResult1">活动优惠：－¥{{couponAmout | two}}</p>
        <p class="total">
        ...
        <div class="bot-confirm">
            ...
            <span @click="payConfirm" :class="(riskbook==true )?'':'pay-color'" v-if="riskbook==true  && checktext!='填写错误' && orderSumit">确认</span>
            <span @click="payBook()" class="pay-color" v-else>确认</span>
        </div>
        <form id="payForm" method="post" :action='proxyConfig'
        @submit.prevent="submitToPay">
...
export default {
  data() {
    return {
      ...
      riskCheck:false,
      InvoiceTitle:null,
      // InvoiceTitle:this.defaultAddress.receiverName,
      orderSumit:true,
    };
  },
  created() {
    ...
    this.orderGoodList.map(function(i, s) {
      if (i != null) {
        ...
        // console.log(s,"sss")
        _this.orderGoodList1.push(i);
      }
    });
    console.log(this.orderGoodList1);
    if (this.orderGoodList1.length == 0) {
      this.$router.push("/home");
    }
    this.orderGoodList1.map(function(i, s) {
    ...
    BuyManager:function(n,o){
      if(n!=null){
        if(n.length==0){
          ...
          this.$http.get("/api/fm/check/"+ this.BuyManager).then(res=>{
              if(res.data.success){
                this.checknum = true;
                this.checktext =null;
              }else{
  ...
  computed: {
    ...
    totalPrice: function() {
      let totalprice = 0;
      this.orderGoodList1.forEach(function(s) {
    ...
    couponAmout:function(){
      ...
      if(this.totalPrice<=1000 && this.couponPostResult){
        ...
        youhui= 0;
        console.log("=0")
      }else if(this.totalPrice> 1000 && this.couponPostResult2){
        youhui = 100;
        console.log("100")
      }
      return youhui
    ...
    //       if(this.BuyManager.length==0){
    //           this.BuyManager =null;
    //       }else if(this.BuyManager.length<8){
    //         text = "填写错误";
    //       }else{
    //         this.$http.get("/api/fm/check/"+ this.BuyManager).then(res=>{
    //           if(res.data.success){
  ...
  methods: {
    ...
    getRisk(){
      this.$http.get('/api/risk/check').then(res=>{
        this.riskCheck = res.data.data;
         if(this.riskCheck == true){
    ...
    chooseAddr(item, i) {
      ...
      this.InvoiceTitle = item.receiverName;
    },
    getaddList() {
      this.$http.get("/api/revieve/address/list").then(res => {
        ...
        this.addList.map(function(i) {
          if (i.isDefault) {
            ...
            _this.InvoiceTitle = i.receiverName;
          }
        });
