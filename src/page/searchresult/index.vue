    ...
    <transition name="slideleft">
        <mt-popup v-model="popupVisible" popup-transition="popup-fade" position="right">
            ...
            <div class="filter-wraper">
                ...
                <ul class="ul-text">
                    <li :class="{isSelected: index==sert}" v-for="(item,index) in sertList" >{{item.name}}<input type="checkbox" @click="sertChose(index)" class="factor-checkbox"></li>
                </ul>
            </div>
            <div class="filter-wraper">
                <p class="factor">材质</p>
...
export default {
  ...
  created() {
    this.getbrandList();
    this.svalue = this.$route.query.id;
    this.$http
      .post("/api/search/", {
        GoodName:this.$route.query.id
      })
      .then(res => {
